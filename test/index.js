...
describe(__filename, function () {
  ...
  it(
    "should throw error when create was not used before request call",
    function (done) {
  ...
  it("should pass configuration to the api impl", function (done) {
    var client = Trooba
      ...
      .use(function tr(pipe) {
        pipe.on("request", function (request) {
          pipe.respond(NodeUtils.format(pipe.context.greeting, request));
        });
      })
      .build()
      .create("client");
  ...
  it(
    "should execute a chain and propagate requestContext implicitly",
    function (done) {
      Trooba
        .use(function handler(pipe) {
  ...
  it(
    "should handle pipe.throw(error) in transport by switching it to reply implicitly",
    function (done) {
      Trooba
        .use(function handler(pipe) {
          pipe.on("request", function () {
  ...
  describe("domain", function () {
    ...
    it(
      "should send error back if no target consumer found for the request by default",
      function (done) {
        var order = [];
        var domain = Domain.create();
        domain.run(function () {
        ...
        domain.once("error", function (err) {
          Assert.equal(
            'No target consumer found for the response "ok"',
            err.message,
          );
    ...
    it(
      "should send error back if no target consumer found when the request is enforced",
      function (done) {
        var order = [];
        var domain = Domain.create();
        domain.run(function () {
        ...
        domain.once("error", function (err) {
          Assert.equal(
            'No target consumer found for the request {"foo":"bar"}',
            err.message,
          );
          Assert.deepEqual(["zx1", "zx2"], order);
          done();
    ...
    it(
      "should send error back if no target consumer found for the response and other types",
      function (done) {
        var order = [];
        var domain = Domain.create();
        domain.run(function () {
    ...
    it(
      "should handle once error and throw error when second one comes",
      function (done) {
        ...
        domain.run(function () {
          pipe.request({})
            .once("error", function (err) {
              Object.keys(pipe.context.$points).forEach(
                function forEach(index) {
                  Assert.deepEqual(
                    {},
                    pipe.context.$points[index]._messageHandlers,
                  );
                },
              );
            });
  ...
  it(
    "should send error back if no target consumer found for the request",
    function (done) {
      var order = [];
      Trooba
        .use(function handler(pipe) {
        ...
        .request({
          ...
          Assert.equal(["zx1", "zx2", "zx3"].toString(), order.toString());
          done();
        });
    },
  );
  ...
  it(
    'should link two pipes via "use", propagate request, response and do trace',
    function (done) {
      var order = [];
      var pipe1 = Trooba
  ...
  it(
    "should not allow hook to the same event in the same handler",
    function (done) {
      var client = Trooba.use(function (pipe) {
        pipe.on("request", function () {
          pipe.respond({});
  ...
  it("should handle once response", function (done) {
    ...
    Object.keys(pipe.context.$points).forEach(function forEach(index) {
      Assert.equal(
        1,
        Object.keys(pipe.context.$points[index]._messageHandlers).length,
      );
    });
  ...
  it(
    "should catch only request chunks and provide hook at stream level",
    function (done) {
      var pipe = Trooba.use(function (pipe) {
        var reqData = [];
  ...
  it("should catch all messages", function (done) {
    ...
    pipe.once("response:end", function (data) {
      Assert.deepEqual(
        ["request", "response", "foo", "bar", undefined],
        messages,
      );
      done();
    });
  ...
  describe("nested pipes", function () {
    it("should build pipe out of main and child pipe", function (next) {
      ...
      Trooba
        ...
        .use(function (pipe) {
          seq.push("main.after");
          Assert.deepEqual(
            "main.before/child.one/child.two/main.after",
            seq.join("/"),
          );
          next();
        })
    ...
    it(
      "should build pipe out of main and a single pipe handler",
      function (next) {
        var seq = [];
        Trooba
          .use(function () {
    ...
    it(
      "should build pipe out of main and child pipe and run ping-pong",
      function (next) {
        var seq = [];
        Trooba
          .use(function (pipe) {
          ...
          .use(function (pipe) {
            pipe.on("request", function () {
              seq.push("main.after");
              Assert.deepEqual(
                "main.before.req/child.one.req/child.two.req/main.after",
                seq.join("/"),
              );
              pipe.respond("pong");
            });
          ...
          .request("ping", function (err, response) {
            Assert.deepEqual(
              "main.before.req/child.one.req/child.two.req/main.after/child.two.res/child.one.res/main.before.res",
              seq.join("/"),
            );
            next();
          });
    ...
    it(
      "should build pipe out of main and a single child handler to run ping-pong",
      function (next) {
        var seq = [];
        Trooba
          .use(function (pipe) {
          ...
          .use(function (pipe) {
            pipe.on("request", function () {
              seq.push("main.after");
              Assert.deepEqual(
                "main.before.req/handler.req/main.after",
                seq.join("/"),
              );
              pipe.respond("pong");
            });
          ...
          .request("ping", function (err, response) {
            Assert.deepEqual(
              "main.before.req/handler.req/main.after/handler.res/main.before.res",
              seq.join("/"),
            );
            next();
          });
  ...
  describe("links", function () {
    ...
    it(
      "should insert one-point pipe to one-point pipe and trace it",
      function (next) {
        order = [];
    ...
    it(
      "should fail to re-insert pipe where it is already added",
      function (next) {
        Assert.throws(function () {
          pipeFoo.next.link(pipeBar);
    ...
    it(
      "should insert long pipe to one-element pipe, context flow",
      function (next) {
        // link to other pipe
        var pipeQaz = Trooba
    ...
    it(
      "should insert long pipe to other long pipe, context flow",
      function (next) {
        pipeFoo = Trooba
          .use(function foo1(pipe) {
            order.push("foo1");
    ...
    describe("propagation through extended pipe", function () {
      ...
      describe(
        "should join two pipes at request stage and route throuh modified pipe",
        function () {
          it("init", function () {
            pipeFoo = Trooba
              .use(function foo1(pipe) {
      ...
      it(
        "should join two pipes at sync message and route to new pipe",
        function (next) {
          var mainPipe = Trooba
            .use(function foo1(pipe) {
